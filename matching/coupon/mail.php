...
$site_top = "https://fukui-knock-luv.com/";

//管理者のメールアドレス ※複数指定する場合は「,」で区切る 例 $to = "aa@aa.aa,bb@bb.bb";
$to = "info@fukui-knock-luv.com";

//送信元メールアドレス ※設置先サイトと同じドメイン推奨
$from = "info@fukui-knock-luv.com";
...
$Referer_check = 1;
$Referer_check_domain = "fukui-knock-luv.com";
...
$jumpPage = 0;
$thanksPage = "https://fukui-knock-luv.com/matching/coupon/thanks.html";

// 必須入力項目を設定 (する=1, しない=0)
$requireCheck = 1;

// 必須入力項目 (入力フォームで指定したname属性の値をカンマで区切って指定)
$require = array('Email', '性別', '年齢', '居住地', '福井県在住または在勤');
      ...
      <div class="container container--notepad">
				<?php if($empty_flag == 1){ ?>
					...
					<div class="form__buttons">
						<button type="button" onClick="history.back()"><img src="../../assets/formBack.png" alt="前画面に戻る"></button>
					</div>
				<?php }else{ ?>
					...
					<form class="form" action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
						...
						<div class="form__buttons">
							...
							<button type="submit"><img src="../../assets/formSubmit.png" alt="送信する"></button>
							<button type="button" onClick="history.back()"><img src="../../assets/formBack.png" alt="前画面に戻る"></button>
						</div>
					</form>
				<?php } ?>
...
<html>
	...
	<body>
		...
		<?php }else{ ?>
			<div class="wrapper">
				<div class="container container--notepad" align="center">
					タップルクーポンを。
						<br>申請いただきありがとうございます。
						<br><small>ご入力いただいたメールアドレスにクーポンコードをお送りいたしますのでご確認のほどよろしくお願いいたします。</small>
						<br><small>※メールは（メールアドレス）より送信されます。</small>
						<br><a class="pageMain__button pageMain__button--backToTop" href="/">
